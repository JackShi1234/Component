...
#import "JMOAuthItem.h"

#import "Masonry.h"

#define JMColorWithRGBA(r, g, b, a) [UIColor colorWithRed:(r/255.0) green:(g/255.0) blue:(b/255.0) alpha:a]
/**
 仅支持键盘输入的UITextView（即禁止粘贴、选择、全选）
 */
@interface JMOnlyKBTextView : UITextView

@end

@implementation JMOnlyKBTextView
...
@interface JMOAuthCode ()<UITextViewDelegate>

@property (nonatomic, strong) UIView *containerView;
@property (nonatomic, strong) JMOnlyKBTextView *textView;
@property (nonatomic, strong) NSArray<JMOAuthItem *> *boxViewArr;

@property (nonatomic, assign) NSInteger maxLenght;

@end

@implementation JMOAuthCode
...
- (instancetype)initWithMaxLength:(NSInteger)maxLength{
    ...
    if (self) {
        ...
        NSLog(@"%ldchushihua",self.maxLenght);
...
- (void)setupDefaultValue{
    //初始化默认值
    self.maxLenght = 2; //初始化再赋值，变了值的数量
    
    self.boxNormalBorderColor = JMColorWithRGBA(255, 255, 255, 0.25);
    self.boxHighlightBorderColor = JMColorWithRGBA(48, 117, 238, 1);
...
- (void)setupUI{
    ...
    for (int index = 0; index < self.maxLenght; index++) {
        ...
        //用于显示边框的View
        JMOAuthItem *boxView = [[JMOAuthItem alloc] init];
...
- (void)updateBoxViewUI{
    ...
    NSInteger index = 0;
    for (JMOAuthItem *boxView in self.boxViewArr) {
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    for (int i= 0; i < self.boxViewArr.count; i++) {
        JMOAuthItem *boxView = self.boxViewArr[i];
...
- (JMOnlyKBTextView *)textView{
    if (!_textView) {
        _textView = [[JMOnlyKBTextView alloc] init];
